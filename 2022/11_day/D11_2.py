with open('11_data.txt', 'r') as file:
...
# Part 2 with updated worry level not being divided by 3
divisors = [monkeys[name]['divisor'] for name in names]
print(divisors)
gcd = 1
    ...
    for name in names:
        for item in monkeys[name]['items']:
            ...
            if monkeys[name]['operation']['operator'] == '*':
                ...
                new = first + second
            # Reset the number if it gets too big
            if new >= gcd:
                new = new % gcd
            #print(new)
