...
char findInDirection(const std::vector<std::string>& floorplanCurrent, int startRow, int startCol, int rowDir, int colDir)
{
    ...
    while (looking)
    {
        currRow += rowDir;
        currCol += colDir;
        if (currRow == -1 || currCol == -1 || currRow == floorplanCurrent.size() || currCol == floorplanCurrent[startRow].size())
        {
            return '.';
        }
        if (floorplanCurrent[currRow][currCol] != '.')
        {
            return floorplanCurrent[currRow][currCol];
        }
    }
    return '!';
}
...
char processSeat(const std::vector<std::string>& floorplanCurrent, int rowInd, int colInd, std::string whereToLook, long& numChanges)
{
    ...
    for (int i = 0; i < whereToLook.size(); i++)
    {
        ...
        if (whereToLook[i] == 'A')
        {
            looking = findInDirection(floorplanCurrent, rowInd, colInd, -1, -1);
        }
        if (whereToLook[i] == 'B')
        {
            looking = findInDirection(floorplanCurrent, rowInd, colInd, -1, 0);
        }
        if (whereToLook[i] == 'C')
        {
            looking = findInDirection(floorplanCurrent, rowInd, colInd, -1, 1);
        ...
        if (whereToLook[i] == 'D')
        {
            looking = findInDirection(floorplanCurrent, rowInd, colInd, 0, -1);
        }
        if (whereToLook[i] == 'E')
        {
            looking = findInDirection(floorplanCurrent, rowInd, colInd, 0, 1);
